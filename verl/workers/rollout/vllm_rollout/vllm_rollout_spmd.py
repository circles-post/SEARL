    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            # if n = 1: (bs, response_length) ; if n > 1: (bs * n, response_length)
                ...
                # Handle all ValueError gracefully to prevent crashes
                logger.warning(
                    f"Caught ValueError: {e}. "
                    f"Returning empty responses filled with pad tokens to continue execution. "
                    f"Batch size: {batch_size}, Response length: {self.config.response_length}"
                )
                # Create empty responses filled with pad tokens
                response = torch.full(
                ...
                # Handle dimension mismatch when n > 1 (multiple samples per prompt)
                if idx.size(0) != response.size(0):
                    repeat_factor = response.size(0) // idx.size(0)
                    ...
                    # Also repeat non_tensor_batch fields if they exist
                    if "tools_kwargs" in non_tensor_batch:
                        non_tensor_batch["tools_kwargs"] = np.repeat(non_tensor_batch["tools_kwargs"], repeat_factor, axis=0)
                    if "question" in non_tensor_batch:
                        non_tensor_batch["question"] = np.repeat(non_tensor_batch["question"], repeat_factor, axis=0)
