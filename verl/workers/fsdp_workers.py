...
from envs.utils.tool_utils_graph import ToolUtilsGraph
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def generate_sequences_loop(self, prompts: DataProto):
        ...
        if self.config.env.mmtool: # for mm tool use
            ...
            requested_tool_utils = str(self.config.env.get("tool_utils_class", "")).strip().lower()

            def _build_rollback_utils(enable_rollback: bool):
                from envs.utils.tool_utils_rollback import ToolUtilsRollback, RollbackConfig
                rollback_config = RollbackConfig(
                    enable_rollback=enable_rollback,
                    max_retries=self.config.env.get("rollback_max_retries", 3),
                    retry_strategy=self.config.env.get("rollback_strategy", "batch_redo"),
                )
                return ToolUtilsRollback(
                    self.tokenizer,
                    meta_info,
                    self.rollout.config,
            ...
            if requested_tool_utils in {"toolutilsrollback", "tool_utils_rollback", "rollback"}:
                enable_rollback = self.config.env.get("enable_rollback", True)
                su = _build_rollback_utils(enable_rollback=enable_rollback)
                logger.info(f"Using ToolUtilsRollback via env.tool_utils_class={requested_tool_utils}, enable_rollback={enable_rollback}")
            elif requested_tool_utils in {"toolutilsgraph", "tool_utils_graph", "graph"}:
                su = ToolUtilsGraph(self.tokenizer, meta_info, self.rollout.config, env_object=self.env_object, is_validate=is_validate)
                logger.info(f"Using ToolUtilsGraph via env.tool_utils_class={requested_tool_utils}")
            elif requested_tool_utils in {"toolutils", "tool_utils", "default", ""}:
                # backward-compatible auto selection
                if self.config.env.get("enable_rollback", False):
                    su = _build_rollback_utils(enable_rollback=True)
                    logger.info("Using ToolUtilsRollback with rollback support")
                elif self.config.env.mcp_enable:
                    su = ToolUtilsGraph(self.tokenizer, meta_info, self.rollout.config, env_object=self.env_object, is_validate=is_validate)
                    logger.info("Using ToolUtilsGraph for MCP tools")
                else:
                    su = ToolUtils(self.tokenizer, meta_info, self.rollout.config, env_object=self.env_object)
                    logger.info("Using standard ToolUtils")
            else:
                logger.warning(f"Unknown env.tool_utils_class={requested_tool_utils}, fallback to auto selection")
                if self.config.env.get("enable_rollback", False):
                    su = _build_rollback_utils(enable_rollback=True)
                    logger.info("Using ToolUtilsRollback with rollback support")
                elif self.config.env.mcp_enable:
        ...
        with self.rollout_sharding_manager:
            ...
            for step in range(max_turns):
                ...
                if 'mcp_worthiness' in prompts.non_tensor_batch:
                    mcp_worthiness = prompts.non_tensor_batch['mcp_worthiness']
                else:
                    mcp_worthiness = None
                if 'tool_call_count' in prompts.non_tensor_batch:
                    tool_call_count = prompts.non_tensor_batch['tool_call_count']
                else:
                    tool_call_count = None
                if tp_rank == 0:
                    if step == 0:
                        if 'question' in prompts.non_tensor_batch:
                            questions = prompts.non_tensor_batch['question']
                        else:
                            questions = None
                        prompts = su.postprocess_output(output, step, questions=questions, mcp_worthiness=mcp_worthiness, tool_call_count=tool_call_count) # 防止subtask prompt未能正常的生成对应subtask
                    else:
                        prompts = su.postprocess_output(output, step, mcp_worthiness=mcp_worthiness)
                    is_finish = prompts is None ### is_finish.
                else:
                    prompts = DataProto()
                    is_finish = None
