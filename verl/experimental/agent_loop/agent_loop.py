...
class AgentLoopOutput(BaseModel):
    ...
    """Extra fields for downstream logging/analysis."""
...
class AgentLoopWorker:
    ...
    def _postprocess(self, inputs: list[AgentLoopOutput]) -> DataProto:
        ...
        extra_fields = [input.extra_fields for input in inputs]
        for key in {k for d in extra_fields for k in d.keys()}:
            non_tensor_batch[key] = np.array([d.get(key) for d in extra_fields], dtype=object)
