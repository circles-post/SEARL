...
class Env(ABC):
    """
    Abstract base class for reinforcement learning environments in the SEARL framework.
    ...
    def step(self, responses, tokenizer=None, loop_cnt=0, questions=None):
    ...
    def compute_score(self, reward_rollout_wg, reward_tokenizer, tokenizer, data: DataProto, if_val=False, use_process_reward=False, mcp_worthiness=None, tool_call_count=None):
        if reward_rollout_wg is not None:
            ...
            score = self._compute_score_with_rules(data, tokenizer, if_val=if_val, mcp_worthiness=mcp_worthiness, tool_call_count=tool_call_count)
            if use_process_reward and not if_val:
                scores = []
                missing_tool_use_scores = 0
                for i in range(len(data)):
                    data_item = data[i]
                    if "tool_use_scores" in data_item.batch.keys():
                        ...
                        # Async rollout path may not produce tool_use_scores.
                        missing_tool_use_scores += 1
                        scores.append(score[i])
                if missing_tool_use_scores > 0:
                    warnings.warn(
                        f"tool_use_scores missing for {missing_tool_use_scores}/{len(data)} samples; "
                        "falling back to final-score-only reward for those samples."
                    )
            else:
                scores = score
