...
class ToolUtils:
    ...
    def postprocess_output(self, output: DataProto, step: int, questions: list=None, mcp_worthiness: list=None, tool_call_count: int=None):
        '''output: cpu'''
        ...
        try:
            ...
            infos_str, dones, _, _ = self.env_object.step(
                responses=responses_str, tokenizer=self.tokenizer, loop_cnt=self.loop_cnt, questions=questions
            )
        #if not use_process_reward will be 0
        print("dones number", sum(dones))
