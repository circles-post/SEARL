...
from .reward_utils import compute_score_baseline
...
class SearchEnv(Env):
    ...
    def compute_score_single(self, data_source, solution_str, ground_truth, extra_info):
        ...
        if data_source in ["MATH-Hard", "jee"]:
            ...
            res_dict = compute_score_baseline(solution_str=solution_str, ground_truth=ground_truth, extra_info=extra_info)
            # Extract score for backward compatibility (compute_score_baseline now returns dict)
    ...
    def _compute_score_with_rules(self, data, tokenizer, if_val=False, mcp_worthiness=None, tool_call_count=None):
        print(f'+++++[info] use reward in SearchEnv [info]+++++')
