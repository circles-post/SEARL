...
export TRAIN_DATA="${TRAIN_DATA:-data/train.parquet}"
export TEST_DATA="${TEST_DATA:-data/val.parquet}"
export RESULT_DIR="${RESULT_DIR:-outputs/checkpoints/grpo}"
...
python3 -m verl.trainer.main_ppo --config-name=rl_factory_ppo_trainer \
    ...
    actor_rollout_ref.env.max_turns=2\
    ...
    trainer.save_freq=1\
    trainer.test_freq=1\
    trainer.total_epochs=1 $@ 2>&1 | tee logs/grpo_debug.log
