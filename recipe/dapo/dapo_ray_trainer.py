                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        with marked_timer("gen_max", timing_raw, "red"):
                            ...
                            gen_baseline_output = self.actor_rollout_wg.generate_sequences_loop(gen_baseline_batch)
                    ...
                    print(f"DEBUG Check: new_batch={len(new_batch.batch)}, gen_output={len(gen_batch_output)}")
                    ...
                    # 将 batch 中的 acc 提前映射到 non_tensor_batch，方便后续 filter 使用
                    if "acc" in new_batch.batch and "acc" not in new_batch.non_tensor_batch:
                        try:
                            metric_arr = new_batch.batch["acc"].cpu().numpy()
                        except Exception:
                            metric_arr = np.array(new_batch.batch["acc"])
                        new_batch.non_tensor_batch["acc"] = metric_arr
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        # 兼容并行reward manager（parallel）将指标放在batch而非non_tensor_batch的情况
                        if metric_name not in new_batch.non_tensor_batch and metric_name in new_batch.batch:
                            try:
                                metric_arr = new_batch.batch[metric_name].cpu().numpy()
                            except Exception:
                                metric_arr = np.array(new_batch.batch[metric_name])
                            new_batch.non_tensor_batch[metric_name] = metric_arr
