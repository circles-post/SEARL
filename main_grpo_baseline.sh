#!/bin/bash
# GRPO Training Script for SEARL
...
export MODEL_PATH="${MODEL_PATH:-/path/to/Qwen/Qwen3-4B}"
export REWARD_MODEL_PATH="${REWARD_MODEL_PATH:-/path/to/Qwen/QwQ-32B}"
export TRAIN_DATA="${TRAIN_DATA:-data/train_baseline.parquet}"
export TEST_DATA="${TEST_DATA:-data/val.parquet}"
export RESULT_DIR="${RESULT_DIR:-outputs/checkpoints/grpo_baseline}"
...
python3 -m verl.trainer.main_ppo --config-name=rl_factory_ppo_trainer \
    ...
    trainer.project_name='GRPO_baseline'\
    trainer.experiment_name='debug_single_gpu'\
    ...
    trainer.total_epochs=1 $@ 2>&1 | tee logs/grpo_baseline_debug.log
